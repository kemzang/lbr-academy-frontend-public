...
const passwordRequirements = [
  { label: '8 caractères minimum', check: (p: string) => p.length >= 8 },
  { label: 'Une majuscule', check: (p: string) => /[A-Z]/.test(p) },
  { label: 'Une minuscule', check: (p: string) => /[a-z]/.test(p) },
  { label: 'Un chiffre', check: (p: string) => /[0-9]/.test(p) },
];
    ...
    <div className="animate-fade-up">
      <div className="text-center mb-8">
        ...
        <p className="text-muted-foreground">
          Rejoignez la communauté des esprits brillants
        </p>
      </div>
            ...
            <div className="grid grid-cols-2 gap-2 mt-3">
              {passwordRequirements.map((req) => {
                ...
                return (
                  ...
                  >
                    <Check className={cn('h-3 w-3', !isMet && 'opacity-50')} />
                    {req.label}
                  </div>
                );
              })}
